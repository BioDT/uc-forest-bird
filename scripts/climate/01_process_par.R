# ==============================================================================
# Script for Processing Climate Data (Photosynthetically Active Radiation - PAR) from NetCDF files
# ==============================================================================
# This script processes PAR data stored in NetCDF format (PAR values in W/m2),
# reprojects the data, crops it to the study region, converts it to Celsius, computes
# the monthly average PARs, and combines the results into a single data frame.
...
process_scenario <- function(scenario) {
  ...
  # Define the input directory where NetCDF climate files (par) are stored
  input_directory <- file.path(project_directory, "data", "climate", scenario, "par")
  ...
  # 4. Converts PAR from W/m2 to mm/month.
  # 5. Extracts time information from the NetCDF data.
  # 6. Computes the monthly average PARs for each file.
  ...
  for (nc_file in nc_files) {
    ...
    # Extract the par values, ignoring the xy coordinates
    par_values <- cropped_df[, 3:ncol(cropped_df)]
    
    # Convert the mm data to a matrix for easier manipulation
    par_matrix <- as.matrix(par_values)
    
    # Compute the column-wise mean par (daily average) across the study region
    ...
    # Combine year, month, and daily par_day_avg for this file into a single data frame
  ...
  # large data frame containing all the processed PAR data for all time periods.
